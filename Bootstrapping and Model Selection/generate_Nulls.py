...
def likelihood(tree, seqs, gtr_probs, gtr_rates):
    def popLeafs(node):
        label = node.get_label(); sequence = seqs[label]; matrix = np.full((4, l), 0) ;i = 0
    ...
    def updateMatrix(node):
        ...
        children = node.child_nodes(); child1 = children[0]; child2 = children[1];
        branch1 = child1.get_edge_length(); branch2 = child2.get_edge_length();
        Q1 = scipy.linalg.expm(R*branch1/d); Q2 = scipy.linalg.expm(R*branch2/d);
        mat1 = matrices[child1]; mat2 = matrices[child2]; matrix = np.zeros((4,l));
        for i in range(0,l): #for each item in the sequence
            for s in range(0,4): #for each row, aka for each A, G, C, T
                #print(s)
                L1 = np.transpose(Q1[s,:])@mat1[:,i]; L2 = np.transpose(Q2[s,:])@mat2[:,i]
                matrix[s,i] = L1 * L2
        matrices[node] = matrix
    ...
    like = 0; pis = np.array([prob_A, prob_G, prob_C, prob_T]); rootMatrix = matrices[root]
    for i in range(0,l):
        temp = np.log(pis@rootMatrix[:,i]); like = like + temp
    #print(like)
    return like
...
def getSample(seqs):
    choosing = np.random.randint(length, size=length)
    newSeqs = dict()
    ...
    for i in choosing:
        for key in keysList:
            newSeqs[key] = newSeqs[key] + (seqs[key][i])
    answer = likelihood(tree, newSeqs, gtr_probs, gtr_rates)
    return answer
...
if __name__ == "__main__":
    ...
    length = len(seqs[ keysList[0] ])   #length of sequences
    numberSamples = 100
    
    #choosing = np.random.randint(length, size=length)
    #for i in range (length):
    #    chosing.append(random.int(range(length)))
    #smol = min(choosing)
    #lorg = max(choosing)
    ...
    distribution = np.zeros((numberSamples))
    
    for i in range(numberSamples):
       distribution[i] = getSample(seqs)
       print(i, distribution[i])
    
    saveDist = distribution.tolist()
    
    #seqs = read_FASTA(infile); infile.close()
    pickle_out = open("dist_10.pickle","wb")
    pickle.dump(saveDist, pickle_out)
    pickle_out.close()
